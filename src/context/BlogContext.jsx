...
export function BlogProvider({ children }) {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[BlogContext] json-server offline, dùng db.json local:', err.message);
      setPosts(DB_FALLBACK.posts);
      setServerOnline(false);
      setError('json-server chưa chạy — đang hiển thị dữ liệu offline từ db.json');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const updatePost = async (id, formData) => {
    ...
    if (serverOnline) {
      ...
      setPosts(prev => prev.map(p => String(p.id) === String(id) ? payload : p));
      return payload;
    }
  };
  ...
  return (
    <BlogContext.Provider value={{
      posts, loading, error, serverOnline,
